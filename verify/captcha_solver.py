...
class JDCaptchaSolver:
    ...
    def check_payload(self, t=''):
        ...
        #     p)
        tk = bridge.encrypt(
            f'{h}{str(len(i)).zfill(4)}{i}{str(len(u)).zfill(4)}{u}{str(len(t)).zfill(6)}{t}{json.dumps(v, separators=(",", ":"))}{random_str(g)}',
            p)
    ...
    def refresh_payload(self):
        ...
        se = bridge.encrypt(json.dumps(f,separators=(",", ":")), u)
        l = {
            'si': r,
            'version': 2,
    ...
    def check_captcha(self,img_dict):
        ...
        # b2_img.show()
        logger.debug(f'b1_size: {b1_img.size}, b2_size: {b2_img.size}')
        bboxes = detector.detection(b1_content)
        logger.debug(f'b1_bbox: {bboxes}')
        if not len(bboxes)>0:
            return
        small_boxes = detector.detection(b2_content)
